...
def get_async_config(cfg: DictConfig) -> Dict[str, Any]:
    ...
    return {
        ...
        "strategy_params": async_cfg.get("strategy_params", async_cfg.get("plora", {})),
    }

def _build_global_model(model_fn, device: torch.device, aggregation_strategy: str, strategy_params: Dict[str, Any]) -> torch.nn.Module:
    if aggregation_strategy == "plora":
        init_seed = strategy_params.get("model_init_seed")
    ...
    if aggregation_strategy == "plora":
        model = inject_lora(
            model,
            rank=int(strategy_params.get("rank", 4)),
            alpha=float(strategy_params.get("alpha", 1.0)),
        )
    return model.to(device)


def _export_global_parameters(model: torch.nn.Module, aggregation_strategy: str) -> List[np.ndarray]:
    if aggregation_strategy == "plora":
        return get_lora_parameters(model)
    return [val.cpu().numpy() for _, val in model.state_dict().items()]


def _import_global_parameters(
    ...
    if aggregation_strategy == "plora":
        set_lora_parameters(model, params)
        return
...
def evaluate_global_model(model: torch.nn.Module, params: List[np.ndarray], test_loader: DataLoader, device: torch.device, aggregation_strategy: str) -> tuple[float, float]:
    """Evaluate model with given parameters."""
    _import_global_parameters(model, params, device, aggregation_strategy)
...
def run_async_simulation(cfg, async_cfg, model_fn, train_loaders, test_loaders, global_test_loader, device, wandb_enabled):
    ...
    aggregation_strategy = str(async_cfg["aggregation_strategy"]).lower()
    strategy_params = async_cfg.get("strategy_params", {})
    ...
    global_model = _build_global_model(model_fn, device, aggregation_strategy, strategy_params)
    global_params = _export_global_parameters(global_model, aggregation_strategy)
    ...
    initial_loss, initial_acc = evaluate_global_model(global_model, global_params, global_test_loader, device, aggregation_strategy)
    
    if wandb_enabled:
        wandb.log(
            {
                "async/global_test_loss": initial_loss,
                "async/global_test_accuracy": initial_acc,
                "async/updates": 0,
...
#        return client_idx, client.fit(FitIns(parameters=params, config=config))
    ...
    while time.time() < end_time and active_tasks:
        ...
        if time.time() - last_eval_time >= waiting_interval:
            ...
            loss, acc = evaluate_global_model(global_model, eval_params, global_test_loader, device, aggregation_strategy)
            print(f"\n[t={time.time() - start_time:.1f}s] Evaluation {eval_counter}: Loss: {loss:.4f}, Accuracy: {acc:.4f}\n")
            
            if wandb_enabled:
                wandb.log(
                    {
                        "async/global_test_loss": loss,
                        "async/global_test_accuracy": acc,
    ...
    final_loss, final_acc = evaluate_global_model(global_model, final_params, global_test_loader, device, aggregation_strategy)
