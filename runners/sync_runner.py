...
def run_sync_simulation(
    ...
    def evaluate_fn(
            ...
            for batch in global_test_loader:
                if isinstance(batch, dict):
                    images, labels = batch.get("img", batch.get("x")).to(
                        device
                    ), batch.get("label", batch.get("y")).to(device)
                else:
        ...
        print(
            f"[Round {server_round} | {elapsed_time:.1f}] Global Eval - Loss: {avg_loss:.4f}, Accuracy: {accuracy:.4f}"
        )
    ...
    final_loss = (
        history.losses_centralized[-1][1] if history.losses_centralized else 0.0
    )
    final_acc = (
        history.metrics_centralized.get("accuracy", [(0, 0.0)])[-1][1]
