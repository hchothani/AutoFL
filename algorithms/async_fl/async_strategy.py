...
class AsynchronousStrategy:
    ...
    def average(
        ...
        elif self.async_aggregation_strategy == "plora":
            return self.weighted_average_plora(
                global_parameters, model_update_parameters, t_diff, num_samples
            )
    ...
    def weighted_average_plora(
        ...
        """Aggregate LoRA-only tensors asynchronously.

        This strategy expects both inputs to contain only LoRA tensors
        (e.g., A/B matrices). It reuses FedAsync weighting for asynchronous
        staleness-aware updates.
        """
        return self.weighted_average_fedasync(
            global_parameters,
