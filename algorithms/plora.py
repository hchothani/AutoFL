...
def inject_lora(model, rank=4, alpha=1.0, _prefix=""):
    ...
    for name, module in model.named_children():
        ...
        if isinstance(module, nn.Linear):
            print(
                f"[LoRA Inject] Replacing {full_name} ({type(module)}) with LoRALinear"
            )
...
class PLoRAStrategy:
    def __init__(
        ...
        **kwargs,
        ...
        self.lora_layers = [
            n for n, m in self.model.named_modules() if isinstance(m, LoRALinear)
        ]
