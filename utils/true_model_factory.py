...
def get_model_classes(cfg):
    ...
    workload_info = get_workload_info(
        cfg.dataset.workload, cfg.cl.strategy, cfg.cl.num_experiences
    )
    ...
    if hasattr(cfg.model, "num_classes") and cfg.model.num_classes != 10:
        print(
            f"manual override: using {cfg.model.num_classes} classes from model config"
        )
...
def create_model(cfg):
    ...
    print(
        f"creating {cfg.model.name} with {model_classes} output classes, {input_channels} input channels, {input_size}x{input_size} input size"
    )
    ...
    if cfg.model.name == "mobilenet":
        ...
        print(
            f"MobileNet config: version={version}, pretrained={pretrained}, classes={model_classes}"
        )
        ...
        model = create_mobilenet(
            num_classes=model_classes, pretrained=pretrained, version=version
        )

        # Validate the created model
        print(
            f"Created MobileNet: {sum(p.numel() for p in model.parameters()):,} parameters"
        )
        return model
        ...
        return create_simple_cnn(
            num_classes=model_classes, in_channels=input_channels, input_size=input_size
        )
...
def validate_config(cfg):
    ...
    if workload_info["task_type"] == "class_incremental" and cfg.cl.strategy in [
        "domain"
    ]:
        warnings.append(
            f"workload '{cfg.dataset.workload}' is class-incremental but using domain strategy"
        )

    # check if too many experiences for split datasets
    if (
        workload_info["task_type"] == "class_incremental"
        and cfg.cl.num_experiences > workload_info["total_classes"]
    ):
        warnings.append(
            f"too many experiences ({cfg.cl.num_experiences}) for {workload_info['total_classes']} classes"
        )

    # check experience replay buffer size
    if hasattr(cfg.cl, "replay_mem_size") and cfg.cl.replay_mem_size > 1000:
        warnings.append(
            f"large replay buffer ({cfg.cl.replay_mem_size}) may cause memory issues"
        )
